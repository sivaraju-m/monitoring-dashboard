...
class DailyReportGenerator:
    ...
    def _print_weekly_summary(self, report: dict):
        ...
        if strategy_analysis:
            print(f"\n🎯 STRATEGY ANALYSIS:")
            print(
                f"   Best Performing: {strategy_analysis.get('best_performing', 'N/A')}"
            )
            print(
                f"   Worst Performing: {strategy_analysis.get('worst_performing', 'N/A')}"
            )
        ...
        if risk_metrics:
            ...
            print(
                f"   Expected Shortfall: ₹{risk_metrics.get('expected_shortfall', 0):,.2f}"
            )

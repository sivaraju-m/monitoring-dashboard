...
class MetricsCollector:
    ...
    def __init__(self):
        ...
        self.endpoints = {
            "strategy_engine": os.getenv("STRATEGY_ENGINE_URL", "https://strategy-engine-uc.a.run.app") + "/api/metrics",
            "trading_engine": os.getenv("TRADING_EXECUTION_ENGINE_URL", "https://trading-execution-engine-uc.a.run.app") + "/api/metrics",
            "data_pipeline": os.getenv("TRADING_DATA_PIPELINE_URL", "https://trading-data-pipeline-uc.a.run.app") + "/api/metrics"
        }
    ...
    async def get_current_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if strategy_metrics:
                metrics.update({
                    "active_strategies": strategy_metrics.get("active_strategies", 0),
    ...
    async def _read_strategy_files(self) -> Dict[str, Any]:
        ...
        try:
            ...
            strategy_dir = Path(__file__).parent.parent.parent.parent.parent / "strategy-engine" / "data"
    ...
    async def _read_trading_files(self) -> Dict[str, Any]:
        ...
        try:
            ...
            trading_dir = Path(__file__).parent.parent.parent.parent.parent / "trading-execution-engine" / "data"
    ...
    async def _store_metrics(self, source: str, metric_type: str, metrics: Dict[str, Any]):
                ...
                for key, value in metrics.items():
                    if isinstance(value, (int, float)):
                        conn.execute("""
                            ...
                            VALUES (?, ?, ?, ?, ?)
                        """, (source, metric_type, key, value, json.dumps({"timestamp": metrics.get("timestamp")})))
    ...
    async def get_alert_count(self) -> int:
        ...
        try:
            ...
            alert_sources = [
                Path(__file__).parent.parent.parent.parent.parent / "strategy-engine" / "logs",
                Path(__file__).parent.parent.parent.parent.parent / "trading-execution-engine" / "logs"
            ]
                ...
                if source_dir.exists():
                    ...
                    for log_file in log_files[:3]:  # Check last 3 log files
                        try:
                            ...
                            for line in lines:
                                if any(level in line.upper() for level in ['ERROR', 'WARNING', 'CRITICAL']):
                                    alert_count += 1
